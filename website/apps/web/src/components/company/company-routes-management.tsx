...
import { reverseMapPlace, searchMapPlaces } from "@/lib/backend-api/maps";
...
async function resolveAddressSuggestionFromText(
  ...
  return {
    ...
    lat: firstResult.lat,
    lng: firstResult.lng,
    source: firstResult.source === "place-index" ? "place-index" : "bootstrap",
  };
}
export function CompanyRoutesManagement({ companyId }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      resolveAddressSuggestionFromText(selectedRoute.startAddress),
      resolveAddressSuggestionFromText(selectedRoute.endAddress),
    ])
      .then(([start, end]) => {
        if (cancelled) {
          return;
  ...
  const handleCopySelectedRouteToDraft = () => {
    ...
    setCreateAllowGuestTracking(
      selectedDraft?.allowGuestTracking ?? selectedRoute.allowGuestTracking,
    );
  ...
  const unifiedWaypoints = useMemo<RouteWaypoint[]>(() => {
    ...
    if (selectedRoute) {
      ...
      // Priority 2: creating a new route
  ...
  const handleMapClick = useCallback(
    async (lat: number, lng: number) => {
      ...
      try {
        const reverseResult = await reverseMapPlace({ lat, lng });
        if (reverseResult) {
          resolvedSuggestion = {
            id: reverseResult.id,
            ...
            source: reverseResult.source === "place-index" ? "place-index" : "bootstrap",
          };
        }
      } catch {
      ...
      if (!resolvedSuggestion) {
        resolvedSuggestion = {
          id: `manual_${lat.toFixed(6)}_${lng.toFixed(6)}`,
      ...
      <div className="w-full space-y-4 lg:w-[45%] lg:min-w-[380px]">
        ...
        {selectedRoute && !showCreatePanel && (
          <RoutePerformanceSummary
            ...
            driverName={routeDriverNames[selectedRoute.routeId] ?? null}
          />
        )}
