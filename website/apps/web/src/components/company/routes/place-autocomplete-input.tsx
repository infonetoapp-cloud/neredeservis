...
function tryParseHistory(rawValue: string | null): AddressSuggestion[] {
  ...
  try {
    ...
    for (const entry of parsed) {
      ...
      if (!id || !label || lat == null || lng == null) continue;
...
function filterHistoryByQuery(historyItems: AddressSuggestion[], query: string): AddressSuggestion[] {
  ...
  if (!normalized) return historyItems.slice(0, 5);
...
function normalizeMapboxFeatures(payload: unknown): AddressSuggestion[] {
  ...
  for (const rawFeature of features) {
    if (typeof rawFeature !== "object" || rawFeature === null || Array.isArray(rawFeature)) continue;
    ...
    if (!placeName || !center || center.length < 2) continue;
...
async function fetchMapboxSuggestions(query: string, signal: AbortSignal): Promise<AddressSuggestion[]> {
  ...
  if (cached && cached.expiresAtMs > nowMs) return cached.items;
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) throw new Error("MAPBOX_AUTH_ERROR");
    if (response.status === 429) throw new Error("MAPBOX_RATE_LIMIT");
...
function mergeSuggestions(historyMatches: AddressSuggestion[], mapboxMatches: AddressSuggestion[]): AddressSuggestion[] {
...
export function AddressAutocompleteInput({
  ...
  useEffect(() => {
    ...
    if (query.length >= 3 && !MAPBOX_PUBLIC_TOKEN) {
      setSuggestions(historyMatches);
      setLoading(false);
      setErrorText("Harita önerileri şu an kapalı. Kayıtlı adreslerden seçim yapabilirsiniz.");
      return;
    }
        ...
        try {
          ...
          if (!controller.signal.aborted) {
            ...
            if (message === "MAPBOX_AUTH_ERROR") {
              setErrorText("Harita servisi yetkisi geçerli değil. Yöneticiye haber verin.");
            } else if (message === "MAPBOX_RATE_LIMIT") {
              setErrorText("Harita sorgu limiti doldu. Kısa süre sonra tekrar deneyin.");
            } else {
              setErrorText("Adres önerisi şu an alınamıyor. Kayıtlı adreslerden devam edebilirsiniz.");
            }
          }
        } finally {
  ...
  const helperText = useMemo(() => {
    if (selectedSuggestion) return `Seçilen adres: ${selectedSuggestion.label}`;
    if (value.trim().length >= 3) return "Listeden bir adres seçerek devam edin.";
    return "Adres bulmak için en az 3 harf yazın.";
  ...
  return (
    <div ref={containerRef} className="relative space-y-1 text-xs font-semibold text-slate-700">
      ...
      <input
        ...
        onKeyDown={(event) => {
          if (event.key !== "Enter" || !onEnterPressed) return;
      ...
      {showSuggestionPanel ? (
        <div className="absolute z-20 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-line bg-white shadow-[0_14px_34px_rgba(15,23,42,0.12)]">
          {loading ? <div className="px-3 py-2 text-xs text-[#667182]">Adres önerileri aranıyor...</div> : null}
          {!loading && suggestions.length === 0 && !errorText ? (
            <div className="px-3 py-2 text-xs text-[#667182]">Öneri bulunamadı.</div>
          ) : null}
          {!loading && errorText ? <div className="px-3 py-2 text-xs text-amber-700">{errorText}</div> : null}
          ...
          {!loading
            ? suggestions.map((suggestion) => (
                ...
                >
                  ...
                  >
                    {suggestion.source === "history" ? "Kayıtlı" : "Harita"}
                  </span>
                </button>
              ))
            : null}
      ...
      {shouldShowHelper ? (
        <div className={`text-[11px] ${required && !selectedSuggestion && open ? "text-amber-700" : "text-[#667182]"}`}>
          {helperText}
        </div>
      ) : null}
    </div>
