...
function softText(value: string | null | undefined, fallback = "Belirtilmemiş"): string {
...
export function RoutePerformanceSummary({
  ...
  const hasVehicle = Boolean(selectedRoute.vehicleId);
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <article className="rounded-2xl border border-line bg-white p-4">
          ...
          <div className="mt-0.5 text-[11px] text-muted">
            {selectedRoute.scheduledTime ?? "Saat belirtilmemiş"}
          </div>
        </article>
          ...
          <div className="mt-1 flex items-center gap-2">
            <div
              className={`h-2.5 w-2.5 rounded-full ${
                hasVehicle ? "bg-emerald-500" : "bg-amber-400"
              }`}
            />
            <span className="text-sm font-semibold text-slate-900">
              {hasVehicle ? selectedRoute.vehiclePlate : "Atanmamış"}
            </span>
          </div>
        </article>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-slate-50/80 px-3 py-3 text-xs text-slate-600">
          ...
          <div className="mt-1 leading-5 text-slate-700">
            {softText(selectedRoute.startAddress)}
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50/80 px-3 py-3 text-xs text-slate-600">
          <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted">
          ...
          <div className="mt-1 leading-5 text-slate-700">{softText(selectedRoute.endAddress)}</div>
        </div>
      </div>

      <div className="mt-3 text-[11px] text-muted">
        Son güncelleme: {formatDateTime(selectedRoute.updatedAt)} |{" "}
        {selectedRoute.allowGuestTracking ? "Misafir takip açık" : "Misafir takip kapalı"} |{" "}
        {selectedRoute.isArchived ? "Arşivlenmiş rota" : "Aktif kayıt"}
      </div>
    </section>
  );
}
