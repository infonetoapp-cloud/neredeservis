...
export default function PlatformCompanyDetailPage() {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      setResetLink(result.loginLink);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Sifre sifirlama linki olusturulamadi.");
    } finally {
      setResetting(false);
    }
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-0.5 text-xs text-muted">
              Sirket sahibi icin yeni bir sifre belirleme linki olusturur.
            </div>
          </div>
          <button
          ...
          >
            {resetting ? "Olusturuluyor..." : "Yeni Giris Linki Olustur"}
          </button>
        </div>
        ...
        {resetLink ? (
          <div className="mt-4 space-y-2">
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2">
              <div className="text-xs font-medium text-emerald-700 mb-1">Giris Linki</div>
              <div className="break-all font-mono text-xs text-emerald-900">{resetLink}</div>
            </div>
            <button
              onClick={handleCopyResetLink}
              className="rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-200 transition"
            >
              {resetCopied ? "Kopyalandi!" : "Kopyala"}
            </button>
          </div>
        ) : null}
      </div>
