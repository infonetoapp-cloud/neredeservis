...
| STG | login page reachable | FAIL | HTTP=ERR; note=Temel al?nan ba?lant? kapat?ld?: G?nderme i?leminde beklenmeyen hata olu?tu. |
| STG | env badge match | FAIL | expected=STG; actual=UNKNOWN |
| STG | google login ui visible | FAIL | Google text not detected in HTML |
| STG | anonymous guard dashboard->login | FAIL | HTTP=ERR; location=-; mode=client_guard_shell |
| STG | public route preview endpoint reachable | FAIL | HTTP=ERR; note=Temel al?nan ba?lant? kapat?ld?: G?nderme i?leminde beklenmeyen hata olu?tu. |
