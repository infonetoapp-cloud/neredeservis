...
import {
  confirmPasswordResetForAddress,
  ...
  verifyPasswordResetCodeForAddress,
...
import { previewMapRoute } from "./lib/map-route-service.js";
import { reverseMapPlace, searchMapPlaces } from "./lib/map-search-service.js";
import {
  inviteCompanyMember,
  removeCompanyMember,
  revokeCompanyInvite,
...
function isMapSearchPath(pathname) {
  return pathname === "/api/maps/search";
}

function isMapReversePath(pathname) {
  return pathname === "/api/maps/reverse";
}

function isMapRoutePath(pathname) {
  return pathname === "/api/maps/route";
}
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (isMapSearchPath(requestUrl.pathname) && request.method === "GET") {
      await requireAuthenticatedUser(request);

      const query = requestUrl.searchParams.get("q") ?? requestUrl.searchParams.get("query") ?? "";
      const limit = requestUrl.searchParams.get("limit");
      const lat = parseOptionalFiniteNumber(requestUrl.searchParams.get("lat"));
      const lng = parseOptionalFiniteNumber(requestUrl.searchParams.get("lng"));
      const result = await searchMapPlaces({
        query,
        limit,
        proximity: lat != null && lng != null ? { lat, lng } : null,
      });
      sendApiOk(response, 200, result);
      return;
    }

    if (isMapReversePath(requestUrl.pathname) && request.method === "GET") {
      await requireAuthenticatedUser(request);

      const lat = parseOptionalFiniteNumber(requestUrl.searchParams.get("lat"));
      const lng = parseOptionalFiniteNumber(requestUrl.searchParams.get("lng"));
      if (lat == null || lng == null) {
        throw new HttpError(400, "invalid-argument", "Reverse geocode icin lat ve lng gerekli.");
      }

      const result = await reverseMapPlace({ lat, lng });
      sendApiOk(response, 200, result);
      return;
    }

    if (isMapRoutePath(requestUrl.pathname) && request.method === "POST") {
      await requireAuthenticatedUser(request);

      const body = await readJsonBody(request);
      const result = await previewMapRoute(asRecord(body) ?? {});
      sendApiOk(response, 200, result);
      return;
    }
    ...
    if (request.method === "POST" && isAuthPasswordResetVerifyPath(requestUrl.pathname)) {
      const body = await readJsonBody(request);
      const result = await verifyPasswordResetCodeForAddress(asRecord(body) ?? {});
      sendApiOk(response, 200, result);
      return;
    }
    ...
    if (request.method === "POST" && isAuthPasswordResetConfirmPath(requestUrl.pathname)) {
      const body = await readJsonBody(request);
      const result = await confirmPasswordResetForAddress(asRecord(body) ?? {});
      sendApiOk(response, 200, result);
      return;
    }
