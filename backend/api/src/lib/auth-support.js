...
function normalizeOobCode(rawValue) {
  if (typeof rawValue !== "string" || rawValue.trim().length === 0) {
    throw new HttpError(400, "invalid-argument", "Gecerli bir reset kodu gereklidir.");
  }

  const oobCode = rawValue.trim();
  if (oobCode.length > 2048) {
    throw new HttpError(400, "invalid-argument", "Reset kodu gecersiz.");
  }

  return oobCode;
}

function normalizeNewPassword(rawValue) {
  if (typeof rawValue !== "string" || rawValue.trim().length === 0) {
    throw new HttpError(400, "auth/missing-password", "Yeni sifre gereklidir.");
  }

  const password = rawValue.trim();
  if (password.length < 8) {
    throw new HttpError(400, "auth/weak-password", "Sifre en az 8 karakter olmalidir.");
  }

  if (password.length > 4096) {
    throw new HttpError(400, "auth/weak-password", "Sifre cok uzun.");
  }

  return password;
}

function readAppWebApiKey() {
  const webApiKey = (process.env.APP_WEB_API_KEY ?? "").trim();
  if (!webApiKey) {
    throw new HttpError(500, "internal", "APP_WEB_API_KEY sunucu degiskeni tanimlanmamis.");
  }
...
async function callIdentityToolkitResetPassword(payload) {
  const webApiKey = readAppWebApiKey();
  const response = await fetch(
    `https://identitytoolkit.googleapis.com/v1/accounts:resetPassword?key=${webApiKey}`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const payloadRecord = asRecord(body);
  if (response.ok) {
  ...
  if (providerCode === "EXPIRED_OOB_CODE") {
    throw new HttpError(400, "auth/expired-action-code", "Bu linkin suresi dolmus.");
  }
  if (providerCode === "INVALID_OOB_CODE") {
    throw new HttpError(400, "auth/invalid-action-code", "Reset linki gecersiz.");
  }
  if (providerCode === "USER_DISABLED") {
    throw new HttpError(403, "auth/user-disabled", "Bu hesap devre disi birakilmis.");
  }
  if (providerCode === "USER_NOT_FOUND" || providerCode === "EMAIL_NOT_FOUND") {
    throw new HttpError(404, "auth/user-not-found", "Bu e-posta ile kayitli hesap bulunamadi.");
  }
  ...
  throw new HttpError(500, "internal", "Sifre sifirlama islemi tamamlanamadi.");
}
...
export async function verifyPasswordResetCodeForAddress(input) {
  const oobCode = normalizeOobCode(input?.oobCode);
  const payload = await callIdentityToolkitResetPassword({ oobCode });
...
export async function confirmPasswordResetForAddress(input) {
  const oobCode = normalizeOobCode(input?.oobCode);
  const newPassword = normalizeNewPassword(input?.newPassword);
  const payload = await callIdentityToolkitResetPassword({
    oobCode,
    newPassword,
  });
